...
function init() {
  ...
  // 移除过时的 physicallyCorrectLights 属性
  ...
  function initGLTFLoader() {
    if (typeof THREE !== 'undefined' && typeof THREE.GLTFLoader !== 'undefined') {
      ...
      console.warn('THREE or GLTFLoader not available yet');
      return false;
    }
  }
...
function handleFileUpload(event) {
  ...
  if (!gltfLoader) {
    ...
    if (typeof THREE !== 'undefined' && typeof THREE.GLTFLoader !== 'undefined') {
      gltfLoader = new THREE.GLTFLoader();
      console.log('GLTFLoader initialized on demand');
    } else {
      alert('3D模型加载器不可用，请刷新页面重试');
